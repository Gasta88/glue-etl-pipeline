...
resource "aws_kinesis_firehose_delivery_stream" "dvault-stream" {
  name        = "dvault-${var.prefix}-stream"
  destination = "extended_s3"

  extended_s3_configuration {
    role_arn   = aws_iam_role.firehose_role.arn
    bucket_arn = aws_s3_bucket.dvault-bucket.arn
  ...
  policy = <<EOF
{
    ...
    "Statement": [
        {      
            ...
            "Resource": [        
                "${aws_s3_bucket.dvault-bucket.arn}",
                "${aws_s3_bucket.dvault-bucket.arn}/*"		    
            ]    
        }, 
        {
            ...
            "Resource": "${aws_kinesis_firehose_delivery_stream.dvault-stream.arn}"
        },
        {
            "Effect"   : "Allow",        
            "Action":  [
...
resource "aws_lambda_function" "lambda_processor" {
  ...
  runtime          = "python3.8"
  timeout          = 300
  source_code_hash = filebase64sha256("../../../shape_dvaults_etl/extraction/firehose_kinesis/lambda_function.zip")
}

#Event bus configuration
resource "aws_cloudwatch_event_bus" "ai-dvault-eventbus" {
  name = "ai-dvault-eventbus-${var.prefix}"
}
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "${aws_kinesis_firehose_delivery_stream.dvault-stream.arn}"
            ]
        }
    ]
}
...
resource "aws_cloudwatch_event_target" "dvault-event-target" {
  arn  = aws_kinesis_firehose_delivery_stream.dvault-stream.arn #Target resource arn
